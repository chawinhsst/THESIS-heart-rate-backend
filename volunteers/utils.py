...
import os
import numpy as np
import requests # <-- ADDED: To download files from URLs
import io       # <-- ADDED: To treat downloaded content as a file

# ==============================================================================
# MAIN DISPATCHER FUNCTION
# ==============================================================================

def analyze_session_file(file_url):
    """
    Analyzes a session file by detecting its extension from the URL
    and calling the appropriate parser.
    """
    # Get the file extension from the URL path, ignoring any query parameters
    file_path = file_url.split('?')[0]
    _, extension = os.path.splitext(file_path)
    extension = extension.lower()

    if extension == '.fit':
        return analyze_fit_file(file_url)
    elif extension == '.tcx':
        return analyze_tcx_file(file_url)
    elif extension == '.csv':
        return analyze_csv_file(file_url)
    else:
...
def analyze_tcx_file(file_url):
    """
    Parses a .tcx file from a URL to extract summary and time-series data.
    """
...
def analyze_csv_file(file_url):
    """
    Parses complex CSV files from a URL by cleaning, transforming, and filling data.
    """
    try:
        # Pandas can read directly from a URL, which is very convenient
        df = pd.read_csv(file_url, skip_blank_lines=True, skipinitialspace=True)
    except Exception as e:
        raise ValueError(f"Failed to read CSV file from URL: {e}")
...
def analyze_fit_file(file_url):
    """
    Parses a .fit file from a URL.
    """
    # --- Download file content from URL ---
    response = requests.get(file_url)
    response.raise_for_status() # Raise an exception for bad status codes
